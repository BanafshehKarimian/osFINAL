...
static void ExecHandler(){
	...
	char file[100];
        ...
        int arg = vaddr;//R_W_Reg(0,4,0);//machine->ReadRegister(4);
/* 	while ((*(char*)&memval) != '\0') {
            file[i]  = (char)memval;
            ++i;
            vaddr++;
	...
	do
	{
		machine->ReadMem(arg + i, 1, (int*)&file[i]);
	}while(file[i++] != '\0');
	...
	OpenFile *exe = fileSystem->Open(file);
        AddrSpace *space;
...
ExceptionHandler(ExceptionType which)
{
    ...
    type=R_W_Reg(0,2,0);
   printf("heeeeeeeeeeeereeeeeee%dtype\n",type);
